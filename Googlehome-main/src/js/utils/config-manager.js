class ConfigManager {
  ...
  static async exportConfig() {
    try {
      ...
      // 获取所有配置数据
      // 书签数据从本地存储读取
      const [settings, squares, searchEngine] = await Promise.all([
        chrome.storage.sync.get('settings'),
        chrome.storage.local.get('squares'),
        chrome.storage.sync.get('searchEngine')
      ]);
      ...
      const data = {
        ...
        squares: squares.squares || [],  // 从本地存储读取书签
  ...
  static async importConfig(file) {
    try {
      ...
      if (config.searchEngine) {
        await chrome.storage.sync.set({ searchEngine: config.searchEngine });
      }
